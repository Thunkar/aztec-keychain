  ...
  protected constructor(value: number | bigint | boolean | BaseField | Buffer) {
    if (Buffer.isBuffer(value)) {
      ...
      if (this.asBigInt >= this.modulus()) {
        throw new Error(
          `Value 0x${this.asBigInt.toString(16)} is greater or equal to field modulus.`
        );
      }
 ...
 */
export function fromBuffer<T extends BaseField>(
 ...
 */
function fromBufferReduce<T extends BaseField>(
  buffer: Buffer,
  f: DerivedField<T>
) {
...
function fromHexString<T extends BaseField>(buf: string, f: DerivedField<T>) {
  ...
  const buffer = Buffer.from(
    checked.length % 2 === 1 ? "0" + checked : checked,
    "hex"
  );
...
export function reduceFn<TInput, TField extends BaseField>(
  fn: (input: TInput) => Buffer,
  field: DerivedField<TField>
) {
