...
import type { AbiType, FunctionAbi } from "./types.js";
import {
  isAddressStruct,
...
class ArgumentEncoder {
    ...
    switch (abiType.kind) {
      ...
      case "struct":
        return abiType.fields.reduce(
          (acc, field) => acc + ArgumentEncoder.typeSize(field.type),
          0
        );
      case "tuple":
        return abiType.fields.reduce(
          (acc, field) => acc + ArgumentEncoder.typeSize(field),
          0
        );
      default: {
  ...
  private encodeArgument(abiType: AbiType, arg: any, name?: string) {
    if (arg === undefined || arg == null) {
      throw new Error(
        `Undefined argument ${name ?? "unnamed"} of type ${abiType.kind}`
      );
    }
    switch (abiType.kind) {
      case "field":
        if (typeof arg === "number") {
          ...
          if (Buffer.isBuffer(arg)) {
            ...
            throw new Error(
              `Argument for ${name} cannot be serialized to a field`
            );
          }
        } else {
      ...
      case "string":
        for (let i = 0; i < abiType.length; i += 1) {
          ...
          const toInsert =
            i < arg.length ? BigInt((arg as string).charCodeAt(i)) : 0n;
...
export function countArgumentsSize(abi: FunctionAbi) {
  return abi.parameters.reduce(
    (acc, parameter) => acc + ArgumentEncoder.typeSize(parameter.type),
    0
  );
}
