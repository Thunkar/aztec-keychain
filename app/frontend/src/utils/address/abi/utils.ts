import type { AbiType } from "./types.js";
...
export function isEthAddressStruct(abiType: AbiType) {
  return (
    abiType.kind === "struct" && abiType.path.endsWith("address::EthAddress")
  );
}
...
export function isAztecAddressStruct(abiType: AbiType) {
  return (
    abiType.kind === "struct" && abiType.path.endsWith("address::AztecAddress")
  );
}
...
export function isFunctionSelectorStruct(abiType: AbiType) {
  return (
    abiType.kind === "struct" &&
    abiType.path.endsWith("types::abis::function_selector::FunctionSelector")
  );
}
